...
Begin VB.Form frm_ListOfBarrowedBooks 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Barrowed Book Records"
   ClientHeight    =   6105
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   10680
   ClipControls    =   0   'False
   Icon            =   "frm_ListOfBarrowedBooks.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6105
   ...
   Begin isCoolButton.isButton btn_Select 
      ...
      _ExtentX        =   2090
      _ExtentY        =   582
      Icon            =   "frm_ListOfBarrowedBooks.frx":3482
      Style           =   5
      Caption         =   "&Select"
      IconAlign       =   1
      iNonThemeStyle  =   0
   ...
   Begin isCoolButton.isButton btn_Close 
      ...
      _ExtentX        =   2090
      _ExtentY        =   582
      Icon            =   "frm_ListOfBarrowedBooks.frx":349E
      Style           =   5
      Caption         =   "&Close"
      IconAlign       =   1
      iNonThemeStyle  =   0
   ...
   Begin VB.Label Label1 
      ...
      Caption         =   "Search books from barrowed to be returned."
      Height          =   510
      Left            =   1215
      TabIndex        =   2
      Top             =   405
   ...
   Begin VB.Image Image1 
      Height          =   720
      Left            =   270
      Picture         =   "frm_ListOfBarrowedBooks.frx":34BA
      Top             =   180
      Width           =   720
   End
End
Attribute VB_Name = "frm_ListOfBarrowedBooks"
...
Private Sub btn_Select_Click()
    ...
    SRC_RI = Val(lvList.SelectedItem.ListSubItems(6))
    
    
        RS_GetListofBooks.Open "SELECT * FROM qry_BarrowedBooks WHERE tbl_BorrowedBooks.RowIndex =" & SRC_RI & "", CN2, adOpenStatic, adLockOptimistic
        
            frm_ReturnBooks.txt_BookID = RS_GetListofBooks.Fields("BookID")
            frm_ReturnBooks.txt_BookTitle = RS_GetListofBooks.Fields("Title")
            frm_ReturnBooks.dtp_DateBarrowed = RS_GetListofBooks.Fields("DateBarrowed")
            frm_ReturnBooks.dtp_DateDue = RS_GetListofBooks.Fields("DateReturned")
            frm_ReturnBooks.txt_Source = RS_GetListofBooks.Fields("BookSource")
            frm_ReturnBooks.txt_Fines = Format(RS_GetListofBooks.Fields("Fines"), "#,##0.00")
            frm_ReturnBooks.BarrowedIndex = SRC_RI
...
CN2.Open "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" & DB_LOCATION & ";Persist Security Info=False;Jet OLEDB:Database Password=admin"
    ...
    RS_ListofBooks.Open "SELECT BookID,Title,DateBarrowed,DateReturned,BookSource,Fines,tbl_BorrowedBooks.RowIndex FROM qry_BarrowedBooks WHERE StudentNo LIKE '" & frm_ReturnBooks.txt_StudentNo.Text & "' AND StatusReturned =" & False & "", CN2, adOpenStatic, adLockOptimistic
...
Call FillListView(lvList, RS_ListofBooks, pos_start, pos_end, 16, 12, False, False)
